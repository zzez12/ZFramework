...
void IMeshBVTree::ExpandNode( IMeshBVTree::IMeshBVNode * pNode )
{
	...
	while ( nIndex < m_nMaxTriangle ) {
		m_pMesh->GetTriangle( m_vTriangles[nIndex].triID, vTri );
		vMeans[0] += vTri[0].X() + vTri[1].X() + vTri[2].X();
		vMeans[1] += vTri[0].Y() + vTri[1].Y() + vTri[2].Y();
		vMeans[2] += vTri[0].Z() + vTri[1].Z() + vTri[2].Z();
		++nCount;
		nIndex = GetNextEntryIdx(pNode, nIndex);
	}
	vMeans *= (1.0f / (3.0f * (float)nCount) );
	...
	// if triangle count <= 3, just do a manual split
	if ( nCount > 3 ) {
