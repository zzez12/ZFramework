			...
			int INFO[], void* otherData)
{
	...
	do
	{
		...
		{
			EVALFUNC_H(N, x, INFO[2] == 0 ? 0 : prev_x, &f, g, m_hessian, otherData);
			HLBFGS_BUILD_HESSIAN_INFO(m_hessian, INFO);
		}
		else if (INFO[2] == 0)
		{
			EVALFUNC(N, x, 0, &f, g, otherData);
			INFO[1]++;
		}
...
#pragma omp parallel for private(i)
		...
		do
		{
			...
			if (blinesearch)
			{
				EVALFUNC(N, x, prev_x, &f, g, otherData);
				INFO[1]++;
			}
		...
		gnorm = HLBFGS_DNRM2(N, g);
		INFO[2]++;
		NEWITERATION(INFO[2], INFO[1], x, &f, g, &gnorm, otherData);
